...
class ShapleyExplainer:
    ...
    def __init__(
        ...
        self.node_feat_norm_params = None
        self.edge_raw_features = edge_raw_features
        self.edge_feat_norm_params = None
        self.time_norm_params = None
        ...
        logger.info(f"  - Estimated model calls: ~{int(nsamples * sample_ratio * 10000)}")
    ...
    def _create_feature_matrix(
        ...
        src_mean_norm = (src_mean - src_mean.min()) / (src_mean.max() - src_mean.min() + 1e-10)
        src_std_norm = (src_std - src_std.min()) / (src_std.max() - src_std.min() + 1e-10)
        src_max_norm = (src_max - src_max.min()) / (src_max.max() - src_max.min() + 1e-10)
        src_min_norm = (src_min - src_min.min()) / (src_min.max() - src_min.min() + 1e-10)
        
        feature_list.extend([src_mean_norm, src_std_norm, src_max_norm, src_min_norm])
        feature_names.extend(['src_node_mean', 'src_node_std', 'src_node_max', 'src_node_min'])
        ...
        dst_mean_norm = (dst_mean - dst_mean.min()) / (dst_mean.max() - dst_mean.min() + 1e-10)
        dst_std_norm = (dst_std - dst_std.min()) / (dst_std.max() - dst_std.min() + 1e-10)
        dst_max_norm = (dst_max - dst_max.min()) / (dst_max.max() - dst_max.min() + 1e-10)
        dst_min_norm = (dst_min - dst_min.min()) / (dst_min.max() - dst_min.min() + 1e-10)
        
        feature_list.extend([dst_mean_norm, dst_std_norm, dst_max_norm, dst_min_norm])
        feature_names.extend(['dst_node_mean', 'dst_node_std', 'dst_node_max', 'dst_node_min'])
        ...
        if edge_ids is not None and len(self.edge_raw_features) > 0:
            ...
            edge_mean_norm = (edge_mean - edge_mean.min()) / (edge_mean.max() - edge_mean.min() + 1e-10)
            edge_std_norm = (edge_std - edge_std.min()) / (edge_std.max() - edge_std.min() + 1e-10)
            edge_max_norm = (edge_max - edge_max.min()) / (edge_max.max() - edge_max.min() + 1e-10)
        ...
        logger.info(f"Created feature matrix with {feature_matrix.shape[1]} aggregated features (reduced from 519 individual features)")
    ...
    def _create_model_wrapper(
        ...
        def wrapper_fn(feature_matrix: np.ndarray) -> np.ndarray:
            ...
            indices = np.array([i if i < len(src_node_ids) else 0 for i in range(batch_size)])
    ...
    def compute_shapley_values(
        ...
        logger.info(f"Sampled {len(self.sampled_indices)} interactions out of {num_total} for explanation")
        ...
        logger.info(f"  - Total model calls: ~{len(self.sampled_indices) * self.nsamples}")
        
        # Choose regularization based on sample/feature ratio
        num_features = explain_data.shape[1]
        if self.num_samples < num_features:
            l1_reg = "num_features(10)"
            logger.info(f"  - Using l1_reg='num_features(10)' (samples={self.num_samples} < features={num_features})")
        else:
        ...
        if non_zero_features:
            logger.info(f"  Top feature: {non_zero_features[0][0]} (importance: {non_zero_features[0][1]:.4f})")
            logger.info(f"  Non-zero features: {len(non_zero_features)}/{len(feature_names)}")
        else:
    ...
    def create_visualizations(
        ...
        filtered_feature_names = [name for name, keep in zip(self.feature_names, non_zero_mask) if keep]
        
        num_filtered = len(self.feature_names) - len(filtered_feature_names)
        if num_filtered > 0:
            logger.info(f"Filtering out {num_filtered} zero-importance features from visualizations")
        ...
        try:
            ...
            plt.title(f"SHAP Summary Plot - {model_name} on {dataset_name}", fontsize=14, pad=20)
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, "shap_beeswarm.png"), dpi=300, bbox_inches="tight")
            plt.close()
        ...
        try:
            ...
            plt.title(f"Feature Importance - {model_name} on {dataset_name}", fontsize=14, pad=20)
            plt.tight_layout()
        ...
        try:
            ...
            top_feature_indices = np.argsort(mean_abs_shap_filtered)[-num_features_show:][::-1]
            
            shap_subset = filtered_shap_values[:num_samples_show, top_feature_indices]
            feature_names_subset = [filtered_feature_names[i] for i in top_feature_indices]
            
            plt.imshow(shap_subset.T, aspect='auto', cmap='RdBu_r', 
                      vmin=-np.abs(shap_subset).max(), vmax=np.abs(shap_subset).max())
            plt.colorbar(label='SHAP value')
            plt.xlabel('Sample Index', fontsize=12)
            plt.ylabel('Feature', fontsize=12)
            plt.yticks(range(len(feature_names_subset)), feature_names_subset, fontsize=8)
            plt.title(f"SHAP Values Heatmap - {model_name} on {dataset_name}", fontsize=14, pad=20)
            plt.tight_layout()
            plt.savefig(os.path.join(save_dir, "shap_heatmap.png"), dpi=300, bbox_inches="tight")
            plt.close()
        ...
        try:
            ...
            plt.title(f"Top {top_n} Features by SHAP Importance - {model_name} on {dataset_name}", 
                     fontsize=14, pad=20)
        ...
        try:
            ...
            axes[0, 0].hist(filtered_shap_values.flatten(), bins=50, color='steelblue', alpha=0.7)
            axes[0, 0].set_xlabel('SHAP Value', fontsize=11)
            axes[0, 0].set_ylabel('Frequency', fontsize=11)
            axes[0, 0].set_title('Distribution of SHAP Values (Non-Zero Features)', fontsize=12)
            ...
            axes[0, 1].set_yticklabels([filtered_feature_names[i] for i in top_10_indices], fontsize=9)
            ...
            axes[1, 0].set_title('Mean SHAP Values (Green=Positive, Red=Negative)', fontsize=12)
            ...
            axes[1, 1].set_yticklabels([filtered_feature_names[i] for i in top_var_indices], fontsize=9)
            axes[1, 1].set_xlabel('Std Dev of SHAP', fontsize=11)
            axes[1, 1].set_title('Top 10 Features by SHAP Variance', fontsize=12)
            axes[1, 1].grid(axis='x', alpha=0.3)
            
            plt.suptitle(f"SHAP Analysis Summary - {model_name} on {dataset_name}", 
                        fontsize=16, y=0.998)
            plt.tight_layout()
            plt.savefig(
