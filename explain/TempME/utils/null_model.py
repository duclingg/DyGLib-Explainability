...
def load_data_shuffle(mode, data):
    ...
    file_path = osp.join(root_dir, 'processed_data', data, 'ml_{}.csv'.format(data))
    g_df = pd.read_csv(file_path)
    ...
    # convert the set union to a list before passing to random.sample
    candidate_nodes = list(set(src_l[ts_l > val_time]).union(set(dst_l[ts_l > val_time])))
    mask_node_set = set(random.sample(candidate_nodes, int(0.1 * num_total_unique_nodes)))
