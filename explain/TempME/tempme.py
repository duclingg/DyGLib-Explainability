...
# Justin Hoang
# TempME for DyGFormer
class TempMEDyGFormer(nn.Module):
    def __init__(
        self,
        base: nn.Module,
        base_model_type: str,
        ...
        self.MLP = nn.Sequential(
            ...
            nn.Linear(self.mlp_dim, self.hid_dim),nn.ReLU(),
            nn.Linear(self.hid_dim, 1)
        )
        self.final_linear = nn.Linear(2 * self.hid_dim, self.hid_dim)
        self.node_emd_dim = self.hid_dim + 12 + self.node_dim if self.if_cat else self.hid_dim + self.node_dim
    ...
    def forward(
        ...
        cut_time_l: np.ndarray, 
        edge_identify: np.ndarray
    ) -> torch.Tensor:
        node_idx, edge_idx, time_idx, cat_feat, _ = walks  # [bsz, n_walk, len_walk]
        edge_features, _ = self.retrieve_edge_features(edge_idx)
        edge_count = torch.from_numpy(edge_identify).float().to(self.device)
        
        # expand edge_count to [bsz, n_walk, len)walk, 3] to match event_dim requirement
        edge_count = edge_count.unsqueeze(-1)  # [bsz, n_walk, len_walk, 1]
        edge_count = edge_count.repeat(1, 1, 1, 3)  # [bsz, n_walk, len_walk, 3]
        ...
        updated_src_feature = self.event_conv(event_features, src_features, tgt_features)  # [bsz, n_walk, len_walk, hid_dim]
        updated_tgt_feature = self.event_conv(event_features, tgt_features, src_features)
        updated_feature = torch.cat([updated_src_feature, updated_tgt_feature], dim=-1)  # [bsz, n_walk, length, hid_dim*2]
        src_feature = self.attention(updated_feature)  # [bsz, n_walks, hid_dim]
        
        if self.if_cat:
            event_cat_f = self.compute_category_features(cat_feat, level="event")  # [bsz, n_walks, 12]
            src_feature = torch.cat([src_feature, event_cat_f], dim=-1)
            
        out = self.MLP(src_feature).sigmoid()
        return out  # [bsz, n_walks, 1]
    
    def enhance_predict_agg(
        self,
        ts_l_cut: np.ndarray,
        walks_src: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
        walks_tgt: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
        walks_bgd: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
        edge_id_info: Tuple[np.ndarray, np.ndarray, np.ndarray], 
        src_gat: torch.Tensor,
        tgt_gat: torch.Tensor,
        bgd_gat: torch.Tensor
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        src_edge, tgt_edge, bgd_edge = edge_id_info
        
        # reduce edge_id_info to [bsz, n_walks, len_walk]
        src_edge = src_edge[:, :, :, 0]
        tgt_edge = tgt_edge[:, :, :, 0]
        bgd_edge = bgd_edge[:, :, :, 0]
        
        # positive pairs (src, tgt)
        src_emb, tgt_emb = self.enhance_predict_pairs(walks_src, walks_tgt, ts_l_cut, src_edge, tgt_edge)
        src_emb = torch.cat([src_emb, src_gat], dim=-1)
        tgt_emb = torch.cat([tgt_emb, tgt_gat], dim=-1)
        pos_score = self.affinity_score(src_emb, tgt_emb)  # [bsz, 1]
        
        # negative pairs (src, bgd)
        src_emb, bgd_emb = self.enhance_predict_pairs(walks_src, walks_bgd, ts_l_cut, src_edge, bgd_edge)
        src_emb = torch.cat([src_emb, src_gat], dim=-1)
        bgd_emb = torch.cat([bgd_emb, bgd_gat], dim=-1)
        neg_score = self.affinity_score(src_emb, bgd_emb)  # [bsz, 1]
    ...
    def enhance_predict_walks(
        self, 
        walks: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray], 
        cut_time_l: np.ndarray, 
        edge_identify: np.ndarray
    ) -> torch.Tensor:
        node_idx, edge_idx, time_idx, cat_feat, _ = walks  # [bdz, n_walk, len_walk]
        edge_features, _ = self.retrieve_edge_features(edge_idx)  # [bdz, n_walk, len_walk, edge_dim]
        edge_count = torch.from_numpy(edge_identify).float().to(self.device)
        
        # expand edge count to [bsz, n_walk, len_walk, 3] to match event_dim requirement
        edge_count = edge_count.unsqueeze(-1)  # [bsz, n_walk, len_walk, 1]
        edge_count = edge_count.repeat(1, 1, 1, 3) # [bsz, n_walk, len_walk, 3]
        time_features = self.retrieve_time_features(cut_time_l, time_idx)
        event_features = torch.cat([edge_features, edge_count, time_features], dim=-1)
        ...
        src_features, tgt_features = self.retrieve_node_features(node_idx)  # [bsz, n_walk, len_walk, node_dim]
        updated_src_feature = self.event_conv(event_features, src_features, tgt_features)  # [bsz, n_walks, length, hid_dim]
        updated_tgt_feature = self.event_conv(event_features, tgt_features, src_features)
        updated_feature = torch.cat([updated_src_feature, updated_tgt_feature], dim=-1)  # [bsz, n_walks, length, hid_dim*2]
        
        src_features = self.attention(updated_feature)  # [bsz, hid_dim]
        src_features = src_features.sum(1)  # [bsz, hid_dim]
        
        if self.if_cat:
            node_cat_f = self.compute_category_features(cat_feat, level="node")
            src_features = torch.cat([src_features, node_cat_f], dim=-1)  #[bsz, hid_dim+12]
        return src_features
    
    def compute_category_features(
        self, cat_feat: np.ndarray, level: str = "node"
    ) -> torch.Tensor:
        cat_feat = torch.from_numpy(cat_feat).long().to(self.device).squeeze(-1)  # [bsz, n_walks]
        cat_feat = torch.nn.functional.one_hot(cat_feat, num_classes=12).to(self.device)  # [bsz, n_walks, 12]
        node_cat_feat = torch.sum(cat_feat, dim=1)  # [bsz, 12]
    ...
    def retrieve_node_features(self, node_ids: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        src_node = torch.from_numpy(node_ids[:, :, [0, 2, 4]]).long().to(self.device)
        tgt_node = torch.from_numpy(node_ids[:, :, [1, 3, 5]]).long().to(self.device)
        src_features = self.node_raw_embed[src_node] # use indexing
        tgt_features = self.node_raw_embed[tgt_node] # use indexing
        return src_features, tgt_features
    
    def retrieve_edge_features(self, edge_idx_records: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        edge_idx_records_th = torch.from_numpy(edge_idx_records).long().to(self.device)
        edge_features = self.edge_raw_embed[edge_idx_records_th]  # using indexing
        masks = (edge_idx_records_th == 0).long().to(self.device)  # [bsz, n_walk] num of null edge in each walk
        masks = masks.unsqueeze(-1)
        return edge_features, masks
    
    def retrieve_time_features(self, cut_time_l: np.ndarray, t_records: np.ndarray) -> torch.Tensor:
        batch = len(cut_time_l)
        t_records_th = torch.from_numpy(t_records).float().to(self.device)
        t_records_th = t_records_th.select(dim=-1, index=-1).unsqueeze(dim=2) - t_records_th
        n_walk, len_walk = t_records_th.size(1), t_records_th.size(2)
    ...
    def retrieve_edge_imp_node(
        self, 
        subgraph: Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]], 
        graphlet_imp: torch.Tensor, 
        walks: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray], 
        training: bool = True
    ) -> Tuple[torch.Tensor, torch.Tensor]: 
        node_record, edge_idx_record, time_record = subgraph
        
        edge_idx_0, edge_idx_1 = edge_idx_record[0], edge_idx_record[1]
        index_tensor_0 = torch.from_numpy(edge_idx_0).long().to(self.device)
        index_tensor_1 = torch.from_numpy(edge_idx_1).long().to(self.device)
    ...
    def retrieve_explanation(
        self, 
        subgraph_src: Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]], 
        graphlet_imp_src: torch.Tensor, 
        walks_src: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
        subgraph_tgt: Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]], 
        graphlet_imp_tgt: torch.Tensor, 
        walks_tgt: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray],
        subgraph_bgd: Tuple[List[np.ndarray], List[np.ndarray], List[np.ndarray]], 
        graphlet_imp_bgd: torch.Tensor, 
        walks_bgd: Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray], 
        training: bool = True
    ) -> List[torch.Tensor]:
        src_0, src_1 = self.retrieve_edge_imp_node(subgraph_src, graphlet_imp_src, walks_src, training=training)
    ...
    def concrete_bern(self, prob: torch.Tensor, training: bool) -> torch.Tensor:
        temp = self.temp
    ...
    def kl_loss(
        ...
        if self.prior == "empirical":
            ...
            kl_loss = (
                (1-s) * torch.log((1 - s) / (1 - target + 1e-6) + 1e-6) + 
                empirical_distribution * torch.log(empirical_distribution / (null_distribution + 1e-6) + 1e-6)
            ).mean()
        else:
            kl_loss = (
                prob * torch.log(prob / target + 1e-6) + 
                (1 - prob) * torch.log((1 - prob) / (1 - target + 1e-6) + 1e-6)
            ).mean()
        return kl_loss