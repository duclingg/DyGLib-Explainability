...
class GraphMixer(nn.Module):
    ...
    def __init__(
        ...
        self.projection_layer = nn.Linear(
            self.edge_feat_dim + time_feat_dim, self.num_channels
        )
    ...
    def compute_node_temporal_embeddings(
        ...
        valid_time_gap_neighbor_node_ids_mask[
            valid_time_gap_neighbor_node_ids_mask == 0
        ] = -1e10
        # Tensor, shape (batch_size, time_gap)
        scores = torch.softmax(valid_time_gap_neighbor_node_ids_mask, dim=1).to(
            self.device
        )
