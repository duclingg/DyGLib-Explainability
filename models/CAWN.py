...
class CAWN(nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(
        ...
        src_node_multi_hop_graphs = self.neighbor_sampler.get_multi_hop_neighbors(
        ...
        # tuple, each element in the tuple is a list of self.walk_length ndarrays, each with shape (batch_size, num_neighbors ** current_hop)
        dst_node_multi_hop_graphs = self.neighbor_sampler.get_multi_hop_neighbors(
            num_hops=self.walk_length,
    ...
    def compute_node_temporal_embeddings(
        ...
        assert (
            nodes_neighbor_times[:, :, 0]
            == node_interact_times.repeat(
                repeats=num_neighbors**self.walk_length, axis=0
            ).reshape(len(node_interact_times), num_neighbors**self.walk_length)
        ).all()
        # ndarray, shape (batch_size, num_neighbors ** self.walk_length, self.walk_length + 1)
        nodes_neighbor_delta_times = (
            nodes_neighbor_times[:, :, 0][:, :, np.newaxis] - nodes_neighbor_times
        )
        ...
        neighbor_time_features = self.time_encoder(
            timestamps=torch.from_numpy(nodes_neighbor_delta_times)
            .float()
            .to(self.device)
            .flatten(start_dim=1)
        ).reshape(
            nodes_neighbor_delta_times.shape[0],
            nodes_neighbor_delta_times.shape[1],
            nodes_neighbor_delta_times.shape[2],
            self.time_feat_dim,
        )
    ...
    def convert_format_from_tree_to_array(
        ...
        nodes_edge_ids = [
            np.zeros((len(node_ids), 1)).astype(np.longlong)
        ] + nodes_edge_ids
        nodes_neighbor_times = [
            node_interact_times[:, np.newaxis]
        ] + nodes_neighbor_times

        array_format_data_list = []
        for tree_format_data in [
            nodes_neighbor_ids,
            nodes_edge_ids,
            nodes_neighbor_times,
        ]:
            ...
            batch_size, num_last_hop_neighbors, walk_length_plus_1, dtype = (
                tree_format_data[0].shape[0],
                tree_format_data[-1].shape[-1],
                len(tree_format_data),
                tree_format_data[0].dtype,
            )
            assert (
                batch_size == len(node_ids)
                and num_last_hop_neighbors == num_neighbors**self.walk_length
                and walk_length_plus_1 == self.walk_length + 1
            )
        ...
        return (
            array_format_data_list[0],
            array_format_data_list[1],
            array_format_data_list[2],
        )
...
class PositionEncoder(nn.Module):
    ...
    def __init__(self, position_feat_dim: int, walk_length: int, device: str = "cpu"):
        ...
        self.position_encode_layer = nn.Sequential(
            nn.Linear(
                in_features=self.walk_length + 1, out_features=self.position_feat_dim
            ),
            nn.ReLU(),
            nn.Linear(
                in_features=self.position_feat_dim, out_features=self.position_feat_dim
            ),
        )
    ...
    def count_nodes_appearances(
        ...
        dst_node_ids: np.ndarray,
        node_interact_times: np.ndarray,
        src_node_multi_hop_graphs: tuple,
        ...
        for idx, (src_node_id, dst_node_id, node_interact_time) in enumerate(
            ...
            src_node_neighbor_ids, src_node_neighbor_times = [
                [src_node_id]
            ] + src_node_neighbor_ids, [[node_interact_time]] + src_node_neighbor_times
            dst_node_neighbor_ids, dst_node_neighbor_times = [
                [dst_node_id]
            ] + dst_node_neighbor_ids, [[node_interact_time]] + dst_node_neighbor_times
            for current_hop in range(self.walk_length + 1):
                for (
                    src_node_neighbor_id,
                    src_node_neighbor_time,
                    dst_node_neighbor_id,
                    dst_node_neighbor_time,
                ) in zip(
                    src_node_neighbor_ids[current_hop],
                    src_node_neighbor_times[current_hop],
                    dst_node_neighbor_ids[current_hop],
                    dst_node_neighbor_times[current_hop],
                ):
                    ...
                    tmp_nodes_appearances[src_node_key][0, current_hop] += (
                        1 / num_current_hop_neighbors
                    )
                    tmp_nodes_appearances[dst_node_key][1, current_hop] += (
                        1 / num_current_hop_neighbors
                    )
    ...
    def forward(self, nodes_neighbor_ids: np.ndarray):
        ...
        batch_indices = (
            np.arange(nodes_neighbor_ids.shape[0])
            .repeat(nodes_neighbor_ids.shape[1] * nodes_neighbor_ids.shape[2])
            .reshape(nodes_neighbor_ids.shape)
        )
        ...
        batch_keys = [
            ...
            for j in range(batch_indices.shape[1])
            for k in range(batch_indices.shape[2])
        ]
        ...
        node_appearances = unique_node_appearances[inverse_indices, :].reshape(
            nodes_neighbor_ids.shape[0],
            nodes_neighbor_ids.shape[1],
            nodes_neighbor_ids.shape[2],
            2,
            self.walk_length + 1,
        )
...
class WalkEncoder(nn.Module):
    ...
    def __init__(
        ...
        if self.attention_dim % self.num_walk_heads != 0:
            self.attention_dim += (
                self.num_walk_heads - self.attention_dim % self.num_walk_heads
            )
        ...
        self.projection_layers = nn.ModuleList(
            [
                nn.Linear(
                    in_features=self.feature_encoder.model_dim
                    + self.position_encoder.model_dim,
    ...
    def forward(
        self,
        neighbor_raw_features: torch.Tensor,
        neighbor_time_features: torch.Tensor,
        edge_features: torch.Tensor,
        neighbor_position_features: torch.Tensor,
        walks_valid_lengths: np.ndarray,
    ):
        """
        ...
        combined_features = self.transformer_encoder(
            inputs_query=combined_features
        ).mean(dim=-2)
        # Tensor, shape (batch_size, self.output_dim)
...
class BiLSTMEncoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, lengths: np.ndarray):
        ...
        encoded_features = encoded_features.reshape(
            encoded_features.shape[0] * encoded_features.shape[1],
