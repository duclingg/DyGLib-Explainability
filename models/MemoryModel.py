...
class MemoryModel(torch.nn.Module):
    ...
    def compute_src_dst_node_temporal_embeddings(
        ...
        updated_node_memories, updated_node_last_updated_times = (
        ...
        if edges_are_positive:
            ...
            unique_src_node_ids, new_src_node_raw_messages = (
            ...
            unique_dst_node_ids, new_dst_node_raw_messages = (
                self.compute_new_node_raw_messages(
...
class MemoryBank(nn.Module):
    ...
    def __init__(self, num_nodes: int, memory_dim: int):
        ...
        self.node_memories = nn.Parameter(
            torch.zeros((self.num_nodes, self.memory_dim)), requires_grad=False
        )
    ...
    def reload_memory_bank(self, backup_memory_bank: tuple):
        ...
        :return:
        """
        self.node_memories.data, self.node_last_updated_times.data = (
...
class MemoryUpdater(nn.Module):
    ...
    def get_updated_memories(
        ...
        if len(unique_node_ids) <= 0:
            return (
                self.memory_bank.node_memories.data.clone(),
    ...
    def compute_node_temporal_embeddings(
        ...
        if current_layer_num == 0:
            ...
            output = self.merge_layers[current_layer_num - 1](
                input_1=output, input_2=node_features
            )

            return output


def compute_src_dst_node_time_shifts(
